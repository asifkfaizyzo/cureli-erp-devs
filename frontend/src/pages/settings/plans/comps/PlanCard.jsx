...
const PlanCard = ({ plan, currentPlan, usage, onSelect, disabled }) => {
  ...
  const getButtonConfig = () => {
    ...
    if (isDark) {
      ...
      return {
        ...
        hoverClasses: "group-hover:bg-[#05015A] group-hover:text-white group-hover:ring-1 group-hover:ring-white/30",
        disabled:     false,
      };
    }
  ...
  const txt = {
    ...
    sub:        isCurrent ? "text-amber-700"    : isDark ? "text-white/60" : "text-gray-600",
    subHover:   (!isCurrent && !isDark) ? "group-hover:text-white/80" : "",
    body:       isCurrent ? "text-amber-700"    : isDark ? "text-white/70" : "text-gray-700",
    bodyHover:  (!isCurrent && !isDark) ? "group-hover:text-white/90" : "",
    muted:      isCurrent ? "text-amber-600/60" : isDark ? "text-white/40" : "text-gray-400",
    mutedHover: (!isCurrent && !isDark) ? "group-hover:text-white/50" : "",
    accent:     isCurrent
                  ? CARD_THEMES.current.accentColor
                  : isDark ? "text-white" : theme.accentColor,
  };
