...
const EditableRow = ({
  ...
  const qtyExceedsStock = row.availableQty > 0 && parseFloat(row.qty) > row.availableQty;
        ...
        <div className="relative">
          <input
            ...
            className={`${inputClass} text-center font-bold ${
              qtyExceedsStock ? "text-red-600 border-red-300 bg-red-50" : "text-amber-700"
            }`}
            placeholder="0"
            min="0"
      ...
      <td className="px-2 py-1.5">
        <div
          className={`h-8 px-2 flex items-center justify-end rounded text-sm font-bold ${
            Number(row.amount) > 0 ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-400"
          }`}
        >
    ...
    <div className="flex-1 flex overflow-hidden relative z-10">
          ...
          <div className="flex items-start gap-3 p-3 rounded-lg bg-gray-50 border border-gray-200">
            ...
            <div>
              ...
              {invoice.customer?.phone && (
                <p className="text-xs text-gray-500 mt-1">{invoice.customer.phone}</p>
              )}
            </div>
          </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-red-700 leading-relaxed mb-3">
                    This invoice has {invoice.returnInvoices.length} approved return(s). You cannot
                    save edits until returns are cancelled.
                  </p>
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-white">
        ...
        <div className="shrink-0 px-6 py-4 border-t border-gray-200 bg-gray-50 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">{/* Reserved for future actions */}</div>
        </div>
      </div>
    </div>
  );
