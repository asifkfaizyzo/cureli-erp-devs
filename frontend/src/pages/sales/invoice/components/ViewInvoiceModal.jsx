...
const ViewInvoiceModal = ({ open, onClose, bill }) => {
  ...
  // 🎯 Use dummy data if bill has no items or doesn't exist
  const displayBill = useMemo(() => {
    ...
    return {
      ...
      items: Array.isArray(bill.items) && bill.items.length > 0 
        ? bill.items 
        : DUMMY_ITEMS,
      billedBy: bill.billedBy || "Admin User",
      time: bill.time || "11:22 AM",
  ...
  if (!open || !displayBill) return null;
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start gap-2 px-3 sm:px-4 py-2 sm:py-2.5 border-b border-gray-100 bg-gray-50/50 shrink-0">
              ...
              <div className="flex items-center gap-1.5 sm:gap-2">
                <button className="flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-4 py-1.5 sm:py-2 bg-white border border-gray-300 rounded-lg text-gray-700 text-[10px] sm:text-sm font-medium hover:bg-gray-50 hover:text-indigo-600 transition-all shadow-sm">
            ...
            <div className="flex-1 overflow-y-auto px-3 sm:px-4 py-2.5 sm:py-3 custom-scrollbar bg-white">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-4 mb-3 sm:mb-4 text-[10px] sm:text-xs text-gray-600 bg-blue-50/50 p-2.5 sm:p-3 rounded-lg border border-blue-100">
                 <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-semibold text-gray-900">{displayBill.billedBy}</span>
                 </div>
                 <div className="w-px h-3 bg-blue-200 hidden sm:block"></div>
                 <div className="flex items-center gap-1.5">
                    <Calendar size={iconSize} className="text-blue-600" />
                    <span className="text-gray-500">Date:</span>
                    <span className="font-semibold text-gray-900">{displayBill.date}</span>
                 </div>
                 <div className="w-px h-3 bg-blue-200 hidden sm:block"></div>
                 <div className="flex items-center gap-1.5">
                    <Clock size={iconSize} className="text-blue-600" />
                    <span className="text-gray-500">Time:</span>
                    <span className="font-semibold text-gray-900">{displayBill.time}</span>
                 </div>
              </div>

              {/* ITEMS TABLE - NO INTERNAL SCROLL, SHOWS ALL ROWS */}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-2.5 sm:gap-3">
                ...
                <div className="lg:col-span-2 bg-gray-50 rounded-xl p-2.5 sm:p-3 border border-gray-200">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-2.5 sm:gap-x-3 gap-y-2">
                    <DetailItem label="Customer ID" value={displayBill.customer.id} size={labelSize} textSize={textSize} />
                    <DetailItem label="Name" value={displayBill.customer.name} size={labelSize} textSize={textSize} />
                    <DetailItem label="Phone" value={displayBill.customer.phone} size={labelSize} textSize={textSize} />
                    <DetailItem label="Doctor" value={displayBill.customer.docName} size={labelSize} textSize={textSize} />
                    <DetailItem label="Payment Mode" value={displayBill.customer.payment} size={labelSize} textSize={textSize} />
                    <DetailItem label="E-Way Bill" value={displayBill.customer.eway || "-"} size={labelSize} textSize={textSize} />
                    ...
                    <div className="col-span-1 sm:col-span-2 md:col-span-3 mt-1 pt-1.5 border-t border-gray-200">
                      <div className="flex gap-1.5 items-start text-gray-600">
                        ...
                        <span className="text-[10px] sm:text-xs leading-snug">{displayBill.customer.address}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 rounded-xl p-2.5 sm:p-3 border border-gray-200 shadow-sm flex flex-col justify-center">
                  ...
                  <div className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
                    <div className="flex justify-between text-gray-500">
                      <span>Sub Total</span>
                      <span className="font-medium text-gray-900">₹{displayBill.summary.subTotal.toLocaleString('en-IN')}</span>
                    </div>
                    <div className="flex justify-between text-gray-500">
                      <span>SGST</span>
                      <span className="font-medium text-gray-900">₹{displayBill.summary.sgst}</span>
                    </div>
                    <div className="flex justify-between text-gray-500">
                      <span>CGST</span>
                      <span className="font-medium text-gray-900">₹{displayBill.summary.cgst}</span>
                    </div>
                    ...
                    <div className="flex justify-between items-end">
                      ...
                      <span className="text-base sm:text-lg font-bold text-[#000060]">
                        ₹{displayBill.summary.total.toLocaleString('en-IN')}
                      </span>
                    </div>
                  </div>
                </div>
...
// Helper Component for Customer Details - Updated
const DetailItem = ({ label, value, size, textSize }) => (
