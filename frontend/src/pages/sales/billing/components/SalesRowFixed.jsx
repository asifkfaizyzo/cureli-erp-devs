...
const SalesRowFixed = memo(forwardRef(({
  index,
  item,
  onChange,
  onProductSelect,
  ...
  useImperativeHandle(ref, () => ({
    ...
    focusLastField: () => {
      ...
      if (lastField) { lastField.focus(); lastField.select?.(); }
    },
    focusField: (fieldKey) => {
    ...
    if (item.inventory_id && item.qty) {
      ...
      if (requestedQty > remainingStock) {
        setStockError({
          ...
          details: usedInOtherRows > 0 ? `(${usedInOtherRows} already in other rows)` : null
        });
      } else {
        setStockError(null);
      }
  ...
  const handleProductSelection = useCallback(async (product) => {
    try {
      ...
      if (availableBatches.length === 1) {
        ...
        if (duplicateIndex !== -1) {
          setBatchError(`This item is already added in Row ${duplicateIndex + 1}`);
  ...
  const handleBatchSelection = useCallback((batch) => {
    ...
    if (duplicateIndex !== -1) {
      setBatchError(`This batch is already added in Row ${duplicateIndex + 1}`);
      return;
    }
  ...
  }, [showProductDropdown, showBatchDropdown, filteredProducts, batches, highlightedIndex, index, isLast, focusNextFieldInRow, onNavigateToNextRow, onNavigateToPrevRow, onCreateNewRow, onRemoveRow, handleProductSelection, handleBatchSelection]);

  useEffect(() => {
    const handleClickOutside = (e) => {
      if (dropdownRef.current && !dropdownRef.current.contains(e.target)) setShowProductDropdown(false);
      if (batchDropdownRef.current && !batchDropdownRef.current.contains(e.target)) setShowBatchDropdown(false);
    };
    ...
    >
      ...
      <td className={`${cellBase} text-center bg-slate-50`}>
        <div className="flex items-center justify-center h-full">
          <span className={`inline-flex items-center justify-center w-5 h-5 rounded text-[8px] font-bold ${hasData ? 'bg-indigo-500 text-white' : 'bg-slate-200 text-slate-500'}`}>
            {rowNumber}
          </span>
        </div>
      </td>
      ...
      {/* BATCH */}
      <td className={`${cellBase} bg-cyan-50/30 relative`} ref={batchDropdownRef}>
        <div className="relative w-full h-full">
          <input
        ...
        <div className={`px-1 py-1 text-center text-[9px] font-medium`}>
          {item.inventory_id ? (
            <div className="flex flex-col">
              <span className={stockError ? 'text-red-600' : parseFloat(item.stock) <= 10 ? 'text-amber-600' : 'text-green-600'}>
                {item.stock || "0"}
              </span>
              {item.inventory_id && allRows.some((r, i) => i !== index && r.inventory_id === item.inventory_id) && (
          ...
          {stockError && <AlertTriangle size={8} className="inline ml-0.5 text-red-600" />}
        </div>
      </td>
