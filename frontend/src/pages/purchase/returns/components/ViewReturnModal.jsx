        ...
        <div className="space-y-3">
          ...
          {returnInvoice.return_reason_notes && (
            <div>
              ...
              <p className="text-sm text-gray-700 italic">"{returnInvoice.return_reason_notes}"</p>
            </div>
          )}
          <div>
            <p className="text-xs text-gray-500 mb-1">Return Date</p>
            <p className="font-medium text-gray-900">{formatDate(returnInvoice.invoice_date)}</p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          {returnInvoice.credit_note_number && (
            <div>
              ...
              <p className="font-mono font-semibold text-emerald-600">{returnInvoice.credit_note_number}</p>
            </div>
          )}
          {returnInvoice.refund_amount && (
            <div>
              <p className="text-xs text-gray-500 mb-1">Refund Amount</p>
              <p className="font-semibold text-red-600">{formatCurrency(returnInvoice.refund_amount)}</p>
            </div>
          )}
          {returnInvoice.refund_notes && (
            <div>
              <p className="text-xs text-gray-500 mb-1">Refund Notes</p>
              <p className="text-sm text-gray-700">{returnInvoice.refund_notes}</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <div className="space-y-2">
              <div>
                ...
                <p className="font-semibold text-gray-900">{returnInvoice.supplier.name}</p>
              </div>
              {returnInvoice.supplier.supplier_code && (
                <div>
                  <p className="text-xs text-gray-500">Supplier Code</p>
                  <p className="font-mono text-sm text-gray-700">{returnInvoice.supplier.supplier_code}</p>
                </div>
              )}
              {returnInvoice.supplier.gst_number && (
                <div>
                  <p className="text-xs text-gray-500">GST Number</p>
                  <p className="font-mono text-sm text-gray-700">{returnInvoice.supplier.gst_number}</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="space-y-2">
              {returnInvoice.supplier.contact_person && (
                <div>
                  ...
                  <p className="font-medium text-gray-900">{returnInvoice.supplier.contact_person}</p>
                </div>
              )}
              {returnInvoice.supplier.office_phone && (
                <div className="flex items-center gap-2">
                  <Phone size={14} className="text-gray-400" />
                  <p className="text-sm text-gray-700">{returnInvoice.supplier.office_phone}</p>
                </div>
              )}
              {returnInvoice.supplier.email && (
                <div className="flex items-center gap-2">
                  <Mail size={14} className="text-gray-400" />
                  <p className="text-sm text-gray-700">{returnInvoice.supplier.email}</p>
                </div>
              )}
            </div>
          </div>
        ...
        {returnInvoice.supplier.address_line_1 && (
          <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <p className="text-sm text-gray-700">
              {returnInvoice.supplier.address_line_1}
              {returnInvoice.supplier.address_line_2 && `, ${returnInvoice.supplier.address_line_2}`}
              {returnInvoice.supplier.city && <br />}
              {returnInvoice.supplier.city && `${returnInvoice.supplier.city}, `}
...
const FinancialTab = ({ returnInvoice }) => (
  <div className="p-6 space-y-6">
    ...
    <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between text-sm">
          ...
          <span className="font-medium text-red-600">-{formatCurrency(returnInvoice.discount_amount)}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-gray-600">Taxable Amount</span>
          <span className="font-medium">{formatCurrency(returnInvoice.taxable_amount)}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-gray-600">CGST</span>
          <span className="font-medium">{formatCurrency(returnInvoice.cgst_amount)}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-gray-600">SGST</span>
          <span className="font-medium">{formatCurrency(returnInvoice.sgst_amount)}</span>
        </div>
        {returnInvoice.igst_amount > 0 && (
          <div className="flex justify-between text-sm">
            <span className="text-gray-600">IGST</span>
            <span className="font-medium">{formatCurrency(returnInvoice.igst_amount)}</span>
          </div>
        )}
        <div className="flex justify-between text-sm">
        ...
        {returnInvoice.round_off !== 0 && (
          <div className="flex justify-between text-sm">
            ...
            <span className="font-medium">{formatCurrency(returnInvoice.round_off)}</span>
          </div>
        )}
    ...
    {/* Credit Notes */}
    {returnInvoice.supplierCredits && returnInvoice.supplierCredits.length > 0 && (
      <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
        <h3 className="text-sm font-semibold text-emerald-700 mb-4">Credit Notes</h3>
        <div className="space-y-3">
              ...
              <div className="shrink-0 px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-[#000060] to-[#000080]">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-xl font-bold text-white">Purchase Return</h2>
...
const PrintContent = ({ returnInvoice }) => (
  <div className="p-8 bg-white">
      ...
      <tbody>
        {returnInvoice.lineItems?.map((item, index) => (
          <tr key={item.item_id}>
            ...
            <td className="border border-gray-300 p-2 text-right">{formatCurrency(item.purchase_rate)}</td>
            <td className="border border-gray-300 p-2 text-right">{formatCurrency(item.line_total)}</td>
          </tr>
        ))}
      </tbody>
    </table>
    ...
    <div className="text-right">
      <p className="text-2xl font-bold">Total: {formatCurrency(Math.abs(returnInvoice.net_amount))}</p>
    </div>
  </div>
);
