...
// Enhanced Searchable Select Component - OPENS UPWARD with ADD NEW SUPPLIER
const SearchableSelect = ({ 
  ...
  displayKey = "name",
  onSelect,
  onAddNew, // NEW: Callback when user wants to add new supplier
  className = "",
  dropDirection = "up"
}) => {
  ...
  // Check if search term matches any existing supplier exactly (case-insensitive)
        ...
        {isOpen ? (
          <input
            ...
            onKeyDown={(e) => {
              ...
              // ✅ ENTER KEY: Select first match OR add new supplier
              if (e.key === 'Enter') {
                e.preventDefault();
                if (filteredOptions.length > 0) {
                  ...
                  // No matches found - open add new supplier modal
          ...
          {search && !exactMatch && onAddNew && (
            <div className="border-t border-gray-100 px-3 py-2 bg-gradient-to-r from-indigo-50 to-purple-50">
              ...
              >
                ...
                <span>Add "<strong>{search}</strong>" as new supplier</span>
                <span className="ml-auto text-[9px] text-indigo-400 bg-white px-1.5 py-0.5 rounded">
                  Press Enter
                </span>
              </button>
            </div>
...
const SupplierDetailsCard = ({ 
  ...
  suppliersList = [], 
  onSupplierSelect,
  onAddNewSupplier // ✅ NEW: Callback to open add supplier modal
}) => {
  ...
  const handleAddNewSupplier = (supplierName) => {
    if (onAddNewSupplier) {
      onAddNewSupplier(supplierName);
    }
  };
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 w-full h-full flex flex-col overflow-visible">
      ...
      <div className="p-4 flex flex-col gap-4 flex-1 overflow-visible">
        ...
        <div className="grid grid-cols-5 gap-3">
          ...
          {/* ✅ Supplier Dropdown - Opens UPWARD with ADD NEW */}
          <SearchableSelect
            label="Supplier Name *"
            ...
            icon={Building2}
            dropDirection="up"
            onAddNew={handleAddNewSupplier} // ✅ NEW: Pass add new handler
            onSelect={(selected) => {
              if (selected) {
