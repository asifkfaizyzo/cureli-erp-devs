...
import { Plus, ChevronUp, ChevronDown, User } from "lucide-react";
...
const SkeletonHeaderStats = () => (
  <div className="shrink-0 bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200 px-3 py-1 flex items-center justify-between">
    <div className="flex items-center gap-3">
      ...
      <div className="w-12 h-4 bg-slate-200 rounded animate-pulse" style={{ animationDelay: '50ms' }} />
    </div>
    <div className="flex items-center gap-2">
      <div className="w-10 h-5 bg-slate-200 rounded animate-pulse" style={{ animationDelay: '100ms' }} />
      <div className="w-14 h-5 bg-slate-200 rounded animate-pulse" style={{ animationDelay: '150ms' }} />
    </div>
  </div>
);
...
const PurchaseTable = ({ 
  ...
  // ✅ Derive billed by name with fallback
  const billedByName = user?.name || user?.full_name || user?.first_name || user?.username || "Staff";
  
  // ✅ DEBUG: Log to verify user data
  useEffect(() => {
    console.log("🔍 [PurchaseTable] User from auth store:", user);
    console.log("🔍 [PurchaseTable] Billed By Name:", billedByName);
  }, [user, billedByName]);
  ...
  const handleProductSelectInternal = useCallback((idx, product) => {
    if (onProductSelect) {
      ...
      {isLoading ? (
        ...
        <div className="shrink-0 bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200 px-3 py-1.5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {/* ✅ BILLED BY BADGE - ALWAYS VISIBLE */}
            <div className="h-3 w-px bg-slate-300" />
            <div className="flex items-center gap-1.5 px-2 py-1 bg-purple-100 rounded-md border border-purple-300">
              <User size={10} className="text-purple-600" />
              <span className="text-[8px] text-purple-600 font-medium">By:</span>
              <span className="text-[9px] font-bold text-purple-800">{billedByName}</span>
            </div>
              ...
              {isLoading ? (
                ...
                rows.map((item, index) => (
                  <PurchaseRowFixed
                    ...
                    onProductSelect={handleProductSelectInternal}
      ...
      <div className="shrink-0 bg-gradient-to-r from-slate-50 to-slate-100 border-t border-slate-200 px-3 py-0.5 flex items-center justify-between text-[8px] text-slate-500">
        {isLoading ? (
          <div className="flex items-center gap-2">
            ...
            <div className="w-12 h-3 bg-slate-200 rounded animate-pulse" style={{ animationDelay: '50ms' }} />
          </div>
        ) : (
          <>
            <div className="flex items-center gap-2">
              ...
              {/* ✅ Also show in footer */}
              <span className="text-slate-300">•</span>
              <span className="text-purple-600 font-medium">By: {billedByName}</span>
            </div>
