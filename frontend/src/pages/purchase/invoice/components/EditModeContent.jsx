...
const EditableRow = ({
  ...
  return (
    ...
    `}>
      ...
      <td className="px-2 py-1.5 relative" ref={dropdownRef}>
        ...
        {showDropdown && filteredProducts.length > 0 && (
          <div className="absolute top-full left-0 z-50 w-80 bg-white border border-amber-200 rounded-lg shadow-xl mt-1 max-h-56 overflow-auto"
            style={{ boxShadow: '0 10px 40px -5px rgba(217, 119, 6, 0.2), 0 4px 6px -2px rgba(0, 0, 0, 0.1)' }}>
            {searchTerm && (
              <div className="px-3 py-2 bg-amber-50 border-b border-amber-100 sticky top-0">
                <p className="text-[10px] text-amber-600 font-medium">Showing {filteredProducts.length} results for "{searchTerm}"</p>
              </div>
            )}
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-xs text-gray-800 truncate">{product.name}</div>
                    <div className="text-[10px] text-gray-500 flex items-center gap-2 mt-0.5">
                      <span className="truncate">{product.manufacturer || '-'}</span>
                      {product.hsn && (<><span>•</span><span className="font-mono">HSN: {product.hsn}</span></>)}
                    </div>
                  </div>
                  {row.medicine_id === product.medicine_id && <CheckCircle2 size={14} className="text-amber-500 shrink-0 ml-2" />}
                </div>
              </div>
            ))}
          </div>
        ...
        {showDropdown && searchTerm && filteredProducts.length === 0 && (
          <div className="absolute top-full left-0 z-50 w-72 bg-white border border-amber-200 rounded-lg shadow-xl mt-1 p-4 text-center">
            ...
            <p className="text-xs text-gray-500">No products found for "{searchTerm}"</p>
          </div>
        )}
      </td>

      <td className="px-2 py-1.5">
        <input type="text" value={row.mfac || ""} onChange={(e) => handleChange("mfac", e.target.value)} className={`${inputClass} text-gray-600`} placeholder="Mfac" />
      </td>
      <td className="px-2 py-1.5">
        <input type="text" value={row.batch || ""} onChange={(e) => handleChange("batch", e.target.value.toUpperCase())} className={`${inputClass} text-center font-mono`} placeholder="Batch" />
      </td>
      <td className="px-2 py-1.5">
        <input type="text" value={row.exp || ""} onChange={(e) => handleChange("exp", e.target.value)} className={`${inputClass} text-center font-mono`} placeholder="MM/YY" maxLength={5} />
      </td>
      <td className="px-2 py-1.5">
        <input type="text" value={row.pack || ""} onChange={(e) => handleChange("pack", e.target.value)} className={`${inputClass} text-center`} placeholder="Pk" />
      </td>
      <td className="px-2 py-1.5">
        <input type="number" value={row.qty || ""} onChange={(e) => handleChange("qty", e.target.value)} className={`${inputClass} text-center font-bold text-amber-700`} placeholder="0" min="0" />
      </td>
      <td className="px-2 py-1.5">
        <input type="number" value={row.sch || row.pQty || ""} onChange={(e) => handleChange("sch", e.target.value)} className={`${inputClass} text-center text-green-600`} placeholder="0" min="0" />
      </td>
      <td className="px-2 py-1.5">
        <input type="number" value={row.price || ""} onChange={(e) => handleChange("price", e.target.value)} className={`${inputClass} text-right`} placeholder="0.00" min="0" step="0.01" />
      </td>
      <td className="px-2 py-1.5">
        <input type="number" value={row.discountPercent || ""} onChange={(e) => handleChange("discountPercent", e.target.value)} className={`${inputClass} text-center`} placeholder="0" min="0" max="100" />
      </td>
      <td className="px-2 py-1.5">
        <input type="number" value={row.sgstPercent || "6"} onChange={(e) => { const val = e.target.value; handleChange("cgstPercent", val); handleChange("sgstPercent", val); }} className={`${inputClass} text-center`} placeholder="6" min="0" max="50" />
      </td>
      <td className="px-2 py-1.5">
        <input type="number" value={row.mrp || ""} onChange={(e) => handleChange("mrp", e.target.value)} className={`${inputClass} text-right font-medium`} placeholder="0.00" min="0" step="0.01" />
      </td>
      <td className="px-2 py-1.5">
      ...
      <td className="px-2 py-1.5 text-center">
        {canRemove && (
          <button onClick={() => onRemove(index)} className="p-1.5 rounded-lg hover:bg-red-100 text-gray-400 hover:text-red-500 transition-all active:scale-95" title="Remove row">
            <X size={14} />
          </button>
        )}
      </td>
  ...
  return (
    <div className="flex-1 flex overflow-hidden relative z-10">
      ...
      <div className="w-80 shrink-0 border-r border-amber-200 flex flex-col overflow-hidden bg-amber-50/30">
        ...
        <div className="shrink-0 p-5 border-b border-amber-200 bg-white">
          ...
          <div className="flex items-start gap-3 p-3 rounded-lg bg-gray-50 border border-gray-200">
            ...
            <div>
              <h3 className="font-semibold text-gray-700">{invoice.supplier?.name || "Unknown"}</h3>
              {invoice.supplier?.gst_number && <p className="text-xs text-gray-500 font-mono mt-1">{invoice.supplier.gst_number}</p>}
            </div>
          </div>
          <p className="text-xs text-amber-600 mt-2 flex items-center gap-1">
            <AlertCircle size={12} />Supplier cannot be changed on existing invoices
          </p>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-white">
        ...
        <div ref={tableBodyRef} className="flex-1 overflow-auto" style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(217, 119, 6, 0.3) transparent" }}>
          {medicinesLoading ? (
