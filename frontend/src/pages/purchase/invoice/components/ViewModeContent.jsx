// frontend/src/pages/purchase/invoice/components/ViewModeContent.jsx
      ...
      {invoice.supplier ? (
        <div className="space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-[#000060]">{invoice.supplier.name}</h3>
              {invoice.supplier.supplier_code && (
                <p className="text-xs text-[#000060]/40 font-mono">{invoice.supplier.supplier_code}</p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-2">
            {(invoice.supplier.office_phone || invoice.supplier.personal_phone) && (
              <div className="flex items-center gap-2 text-sm text-[#000060]/70">
                ...
                <span>{invoice.supplier.office_phone || invoice.supplier.personal_phone}</span>
              </div>
            )}
          ...
          {(invoice.supplier.address_line_1 || invoice.supplier.city) && (
            <div className="flex gap-2 pt-3 border-t border-[#000060]/10">
              ...
              <p className="text-xs text-[#000060]/50 leading-relaxed">
                {[invoice.supplier.address_line_1, invoice.supplier.city, invoice.supplier.state, invoice.supplier.pincode]
                  .filter(Boolean)
                  .join(", ")}
              </p>
            </div>
    ...
    >
      ...
      <div className="space-y-3">
        ...
        {parseFloat(invoice.discount_amount) > 0 && (
          <FinanceRow label="Discount" value={`- ${formatCurrency(invoice.discount_amount)}`} valueClass="text-[#000060]/80" />
        )}
        ...
        {parseFloat(invoice.cgst_amount) > 0 && <FinanceRow label="CGST" value={formatCurrency(invoice.cgst_amount)} small />}
        {parseFloat(invoice.sgst_amount) > 0 && <FinanceRow label="SGST" value={formatCurrency(invoice.sgst_amount)} small />}
        {parseFloat(invoice.igst_amount) > 0 && <FinanceRow label="IGST" value={formatCurrency(invoice.igst_amount)} small />}
        <FinanceRow label="Total Tax" value={formatCurrency(invoice.total_tax)} valueClass="text-[#000060]/90" />
        {invoice.round_off !== 0 && <FinanceRow label="Round Off" value={formatCurrency(invoice.round_off)} small />}
        <div className="border-t-2 border-[#000060]/20 my-4" />
        <div className="flex justify-between items-center py-2">
          <span className="font-semibold text-[#000060]">Net Amount</span>
          <span className="text-2xl font-bold text-[#000060]">{formatCurrency(invoice.net_amount)}</span>
        </div>
        ...
        <tbody className="divide-y divide-[#000060]/5">
          {invoice.lineItems && invoice.lineItems.length > 0 ? (
            invoice.lineItems.map((item, i) => {
              ...
              const isExpiringSoon = item.expiry_date && new Date(item.expiry_date) < new Date(Date.now() + 90 * 24 * 60 * 60 * 1000);
              ...
              return (
                <tr key={item.item_id || `item-${i}`} className="hover:bg-[#000060]/[0.02] transition-colors group">
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className="font-mono text-xs px-2 py-1 rounded bg-[#000060]/5 text-[#000060]/70 border border-[#000060]/10">{item.batch_number || "-"}</span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <span className={`text-xs ${isExpiringSoon ? "text-red-600 font-semibold bg-red-50 px-2 py-0.5 rounded" : "text-[#000060]/60"}`}>
                      {formatDate(item.expiry_date)}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center text-xs text-[#000060]/50">{item.pack_size || "-"}</td>
                  <td className="px-4 py-3 text-right"><span className="font-bold text-[#000060]">{parseFloat(item.quantity) || 0}</span></td>
                  <td className="px-4 py-3 text-right"><span className="font-medium text-[#000060]/70">{parseFloat(item.free_quantity) || 0}</span></td>
                  <td className="px-4 py-3 text-right text-sm text-[#000060]/70">{formatCurrency(item.purchase_rate)}</td>
                  <td className="px-4 py-3 text-right text-sm font-semibold text-[#000060]">{formatCurrency(item.mrp)}</td>
                  <td className="px-4 py-3 text-center">
                    {(parseFloat(item.trade_discount) || 0) > 0 ? (
                      <span className="text-xs font-semibold text-[#000060]/80">{item.trade_discount}%</span>
                    ) : (
                      <span className="text-xs text-[#000060]/30">-</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-center"><span className="text-xs font-medium text-[#000060]/70">{gstPercent.toFixed(0)}%</span></td>
                  <td className="px-4 py-3 text-right"><span className="font-bold text-[#000060]">{formatCurrency(item.line_total)}</span></td>
                </tr>
              );
            })
          ) : (
        ...
        {invoice.lineItems && invoice.lineItems.length > 0 && (
          <tfoot className="sticky bottom-0 border-t-2 border-[#000060]/20 bg-white">
            <tr className="text-sm font-semibold">
              <td colSpan={5} className="px-4 py-4 text-right text-[#000060]/60">Grand Totals</td>
