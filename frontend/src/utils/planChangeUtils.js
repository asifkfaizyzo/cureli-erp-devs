// frontend/src/utils/planChangeUtils.js
 ...
 */
export function checkCompliance(targetPlan, activeUsersAfterDisable, activeBranchesAfterDeactivate) {
  const maxUsers = targetPlan.max_users === -1 ? Infinity : targetPlan.max_users;
  const maxBranches = targetPlan.max_branches === -1 ? Infinity : targetPlan.max_branches;
...
export function getPlanComparisonSummary(currentPlan, targetPlan) {
  ...
  if (currentUsers !== targetUsers) {
    ...
    const isIncrease = normalizeLimit(targetUsers) > normalizeLimit(currentUsers);
  ...
  if (currentBranches !== targetBranches) {
    ...
    const isIncrease = normalizeLimit(targetBranches) > normalizeLimit(currentBranches);
