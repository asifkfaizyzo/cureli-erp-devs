...
async function main() {
  ...
  // ============================================
// 4A. CREATE MANUAL SUPER ADMINS (ASIF & KIRAN)
// ============================================

console.log("👑 Creating Manual Super Admin Shops...");
...
const manualSAs = [
  {
    ...
    phone: "9961045596",
    password: "Qwerty@11",
    shop_name: "Asif Health Pharmacy",
    cityIndex: 0,
  },
  {
    first_name: "Kiran",
    ...
    phone: "8086415357",
    password: "ACHUgopi@123",
    shop_name: "Kiran Medicals",
    cityIndex: 1,
  },
];

for (const sa of manualSAs) {
  const location = INDIAN_CITIES[sa.cityIndex];
  const saPasswordHash = await bcrypt.hash(sa.password, 10);
  ...
  const superAdmin = await prisma.user.create({
    data: {
      ...
      full_name: `${sa.first_name} ${sa.last_name}`,
      username: sa.username,
      email: sa.email,
      phone_number: `${sa.phone}`,
      password_hash: saPasswordHash,
      login_provider: "password",
      role: "super_admin",
      status: "verified",
      is_active: true,
  ...
  const shop = await prisma.shop.create({
    data: {
      shop_id: uuid(),
      owner_user_id: superAdmin.user_id,
      business_name: sa.shop_name,
      legal_name: `${sa.shop_name} Pvt Ltd`,
      business_type: "Private Limited",
      gst_number: `KL${randomBetween(10,99)}ABCDE${randomBetween(1000,9999)}Z5`,
  ...
  const branch = await prisma.branch.create({
    data: {
      branch_id: uuid(),
      shop_id: shop.shop_id,
      branch_name: `${sa.shop_name} - Main`,
      branch_type: "main",
      address_line_1: shop.address_line_1,
      city: location.city,
      state: location.state,
      pincode: location.pincode,
      contact_number: `+91${sa.phone}`,
      is_active: true,
      created_at: shop.created_at,
    },
  });
  ...
  // Attach Starter Plan
  const starter = preMadePlans.starter;
  ...
  const subscription = await prisma.shopSubscription.create({
    data: {
      ...
      end_date: daysFromNow(360),
      renewal_date: daysFromNow(330),
      branch_limit_snapshot: starter.max_branches,
      user_limit_snapshot: starter.max_users,
      is_active: true,
  ...
  await prisma.user.create({
    data: {
      ...
      first_name: "Badmin",
      last_name: sa.first_name,
      full_name: `Badmin ${sa.first_name}`,
      username: `badmin_${sa.username}`,
      email: `badmin_${sa.username}@example.com`,
      phone_number: `+9190${randomBetween(10000000,99999999)}`,
      password_hash: saPasswordHash,
      login_provider: "password",
      role: "admin",
      status: "verified",
  ...
  await prisma.user.create({
    data: {
      ...
      first_name: "Staff",
      last_name: sa.first_name,
      full_name: `Staff ${sa.first_name}`,
      username: `staff_${sa.username}`,
      email: `staff_${sa.username}@example.com`,
      phone_number: `+9191${randomBetween(10000000,99999999)}`,
      password_hash: saPasswordHash,
      login_provider: "password",
      role: "staff",
      status: "verified",
...
console.log("✅ Created Asif & Kiran Super Admin setups\n");
