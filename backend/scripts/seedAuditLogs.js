...
const ACTIONS = [
  ...
  },
  {
    ...
    metadata: {
      reason: "Image blurry",
      rejection_note: "Please upload a clearer scan",
    },
  },
...
async function main() {
  ...
  try {
    ...
    const cadmins = await prisma.cAdmin.findMany({
      take: 5,
      select: { cadmin_id: true, role: true },
    });

    userIds =
      users.length > 0 ? users : [{ user_id: uuid(), role: "branch_admin" }];
    shopIds = shops.length > 0 ? shops : [{ shop_id: uuid() }];
    branchIds = branches.length > 0 ? branches : [{ branch_id: uuid() }];
    cadminIds =
      cadmins.length > 0
        ? cadmins
        : [{ cadmin_id: uuid(), role: "SUPER_ADMIN" }];

    console.log(
      ` Loaded context: ${users.length} users, ${shops.length} shops, ${cadmins.length} admins`,
    );
  } catch (e) {
  ...
  for (let i = 0; i < LOG_COUNT; i++) {
    ...
    if (template.entity_type === "branch")
      entityId = randomItem(branchIds).branch_id;
    if (template.entity_type === "cadmin")
      entityId = randomItem(cadminIds).cadmin_id;
