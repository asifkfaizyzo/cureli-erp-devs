...
import { S3Client, PutObjectAclCommand, PutBucketPolicyCommand, GetBucketPolicyCommand } from '@aws-sdk/client-s3';
...
const BUCKET_NAME = process.env.AWS_S3_BUCKET_NAME || process.env.AWS_S3_BUCKET || 'cureli-prod-assets';
...
async function addBucketPolicy() {
  ...
  try {
    ...
    try {
      const existingPolicy = await s3Client.send(new GetBucketPolicyCommand({ Bucket: BUCKET_NAME }));
      ...
      const hasOurPolicy = existing.Statement.some(s => s.Sid === 'PublicReadEmailAssets');
...
async function main() {
  ...
  console.log(`${`https://${BUCKET_NAME}.s3.${REGION}.amazonaws.com/email-assets/cureli-logo-dark.png`}\n`);
}
