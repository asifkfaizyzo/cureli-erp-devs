...
async function copyImages() {
  ...
  try {
    ...
    for (const skuId of skuFolders) {
      ...
      const highResImages = files.filter((file) =>
        IMAGE_PATTERNS.high.test(file),
      );
      ...
      if (stats.foldersProcessed % 100 === 0) {
        process.stdout.write(
          `\r✓ Processed ${stats.foldersProcessed} folders... (${stats.imagesCopied} images copied)`,
        );
      }
    }
    ...
    console.log("─".repeat(60));
    console.log(
      `   Folders processed:    ${stats.foldersProcessed.toLocaleString()}`,
    );
    console.log(
      `   Images found:         ${stats.imagesFound.toLocaleString()}`,
    );
    console.log(
      `   Images copied:        ${stats.imagesCopied.toLocaleString()}`,
    );
    console.log(
      `   Images skipped:       ${stats.imagesSkipped.toLocaleString()}`,
    );
