...
const generateEnquiryNumber = async () => {
  ...
  const lastEnquiry = await prisma.enquiry.findFirst({
    where: {
      enquiry_number: { startsWith: prefix },
    },
...
export const listEnquiries = async (options) => {
  ...
  // Search by name, email, or enquiry number
  if (search) {
    where.OR = [
  ...
  // ✅ FIXED: Use _count for accurate reply count
...
export const replyToEnquiry = async (enquiryId, adminId, data) => {
  ...
  // Create reply record in transaction
...
//   // Create reply and update enquiry status in transaction
//   let emailSent = false;
//   let emailError = null;
