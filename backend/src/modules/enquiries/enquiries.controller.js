...
export const submitEnquiry = async (req, res) => {
  try {
    const { name, email, phone, message, recaptchaToken } = req.validated?.body || req.body;
    ...
    // ✅ FIXED: Verify reCAPTCHA (uncomment for production)
...
export const listEnquiries = async (req, res) => {
  try {
    const { page, limit, status, search, sortBy, sortOrder } = req.validated?.query || req.query;
...
export const getEnquiryDetails = async (req, res) => {
  try {
    const { enquiryId } = req.validated?.params || req.params;
...
export const replyToEnquiry = async (req, res) => {
  try {
    const { enquiryId } = req.validated?.params || req.params;
    const { subject, message } = req.validated?.body || req.body;
