...
// ============================================
// TICKET STATUS CHANGED EMAIL TEMPLATE - DARK MODE FIXED
// ============================================

import { EMAIL_CONFIG, getBaseHeadContent, renderLogo, renderFooter, renderButton, getSupportLink } from './_helpers.js';

const STATUS_DISPLAY = {
  PENDING: { label: 'Pending', color: EMAIL_CONFIG.COLORS.WARNING, bgColor: '#fef3c7', darkBg: '#44403c', darkText: '#fcd34d', message: 'Your ticket is awaiting review by our support team.' },
  IN_PROGRESS: { label: 'In Progress', color: '#3b82f6', bgColor: '#dbeafe', darkBg: '#1e3a5f', darkText: '#93c5fd', message: 'Our support team is actively working on your ticket.' },
  RESOLVED: { label: 'Resolved', color: EMAIL_CONFIG.COLORS.SUCCESS_LIGHT, bgColor: '#d1fae5', darkBg: '#064e3b', darkText: '#6ee7b7', message: "Your issue has been resolved. If you're still experiencing problems, you can reopen this ticket." },
  CLOSED: { label: 'Closed', color: EMAIL_CONFIG.COLORS.GRAY, bgColor: '#f3f4f6', darkBg: '#374151', darkText: '#d1d5db', message: 'This ticket has been closed. Thank you for contacting support.' },
  CANCELLED: { label: 'Cancelled', color: EMAIL_CONFIG.COLORS.ERROR, bgColor: '#fee2e2', darkBg: '#450a0a', darkText: '#fca5a5', message: 'This ticket has been cancelled.' },
};
  ...
  const statusChangeHtml = fromConfig
    ? `
      <div style="text-align:center;margin:24px 0;">
        <span style="display:inline-block;padding:8px 18px;border-radius:20px;font-size:13px;font-weight:600;background-color:${fromConfig.bgColor};color:${fromConfig.color};">${fromConfig.label}</span>
        <span style="color:#9ca3af;font-size:24px;margin:0 12px;font-weight:300;">→</span>
        <span style="display:inline-block;padding:8px 18px;border-radius:20px;font-size:13px;font-weight:700;background-color:${toConfig.bgColor};color:${toConfig.color};">${toConfig.label}</span>
      </div>
    `
    : `
      <div style="text-align:center;margin:24px 0;">
        <span style="display:inline-block;padding:10px 24px;border-radius:20px;font-size:15px;font-weight:700;background-color:${toConfig.bgColor};color:${toConfig.color};">${toConfig.label}</span>
      </div>
    `;
  ...
  <div class="container" style="max-width:560px;margin:0 auto;padding:20px;">
    ...
    <div class="content-bg content" style="background-color:#ffffff;padding:32px;border:1px solid #e5e7eb;border-top:none;">
      ...
      <div class="card-bg" style="background-color:#f9fafb;border:1px solid #e5e7eb;border-radius:10px;padding:18px 20px;margin-bottom:24px;">
        <table style="width:100%;border-collapse:collapse;">
          <tr>
            ...
            <td class="ticket-code" style="padding:6px 0;font-weight:700;font-size:14px;color:${EMAIL_CONFIG.COLORS.PRIMARY};font-family:'Courier New',monospace;background-color:#f9fafb;">${ticket_number}</td>
          </tr>
          <tr>
            <td class="table-label" style="padding:6px 0;color:#6b7280;font-size:13px;background-color:#f9fafb;">Subject</td>
            <td class="table-value" style="padding:6px 0;color:#1f2937;font-weight:600;font-size:14px;background-color:#f9fafb;">${ticketSubject || 'N/A'}</td>
          </tr>
        </table>
      </div>
      ...
      <div class="status-box" style="background-color:#f9fafb;border-radius:10px;padding:20px;margin:24px 0;">
        <p class="table-label" style="margin:0 0 12px;color:#6b7280;font-size:12px;text-transform:uppercase;letter-spacing:0.5px;text-align:center;font-weight:600;background-color:#f9fafb;">Status Update</p>
        ${statusChangeHtml}
        <p class="text-secondary" style="margin:16px 0 0;color:#4b5563;font-size:14px;text-align:center;line-height:1.6;background-color:#f9fafb;">${toConfig.message}</p>
      </div>
