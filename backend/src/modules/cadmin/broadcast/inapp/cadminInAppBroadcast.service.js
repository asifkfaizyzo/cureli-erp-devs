...
async function countNotificationsByCampaign(campaignId, readOnly = false) {
  try {
    ...
    console.error("[Broadcast] countNotificationsByCampaign failed:", err.message);
    return 0;
  }
}
...
function validateFilters(filters) {
  ...
  const { shop_ids, plan_ids, roles, registration_date_from, registration_date_to } = filters;
...
export async function previewRecipientCount(filters, includeDetails = false) {
  try {
    ...
    console.log("[Broadcast Preview] audienceFilters:", JSON.stringify(audienceFilters));
    ...
    if (shopIds.length > 0 && includeDetails) {
      ...
      recipients.forEach((r) => {
        if (r.shop_id) {
          if (!shopDetails[r.shop_id]) shopDetails[r.shop_id] = { name: null, count: 0 };
...
export async function sendImmediate(data, auditContext = {}) {
  ...
  try {
    ...
    const { cleanFilters, includeUsers, includeCAdmins } =
      extractAudienceFlags(target_filters, target_users, target_cadmins);
...
export async function createDraft(data, auditContext = {}) {
  ...
  try {
    ...
    }).catch((err) => console.error("[AUDIT] Failed to log inapp draft creation:", err));
    ...
    return {
      ...
      attachments:     formatAttachmentsForAPI(campaignWithAttachments.attachmentFiles),
...
export async function updateDraft(campaignId, data, auditContext = {}) {
  try {
    ...
    if (existing.status === "sent")    throw createError("Cannot edit a sent campaign", 400);
    if (existing.status === "cancelled") throw createError("Cannot edit a cancelled campaign", 400);
    ...
    if (data.message !== undefined) {
      if (data.message.trim().length < 10) throw createError("Message must be at least 10 characters");
      if (data.message.length > 500)       throw createError("Message must not exceed 500 characters");
    ...
    if (data.target_filters !== undefined) {
      ...
      const { cleanFilters, includeUsers, includeCAdmins } =
        extractAudienceFlags(data.target_filters, topLevelUsers, topLevelCAdmins);
    ...
    }).catch((err) => console.error("[AUDIT] Failed to log inapp draft update:", err));
...
export async function scheduleBroadcast(campaignId, scheduledFor, auditContext = {}) {
  try {
    ...
    if (!existing)                   throw createError("Campaign not found", 404);
    if (existing.status !== "draft") throw createError("Only draft campaigns can be scheduled", 400);
    ...
    }).catch((err) => console.error("[AUDIT] Failed to log inapp schedule:", err));
...
export async function sendScheduled(campaignId) {
  try {
    ...
    if (campaign.status !== "scheduled") throw createError("Campaign is not scheduled", 400);
    ...
    if (recipients.length === 0) {
      console.warn(`[Broadcast Service] No recipients for campaign ${campaignId}`);
      await prisma.broadcastCampaign.update({
        where: { campaign_id: campaignId },
    ...
    await prisma.broadcastCampaign.update({
      where: { campaign_id: campaignId },
      data:  { status: "sent", sent_at: new Date(), delivered_count: deliveredCount },
    });
    ...
    }).catch((err) => console.error("[AUDIT] Failed to log scheduled inapp send:", err));
    ...
    console.error(`[Broadcast Service] Send scheduled ${campaignId} failed:`, error);
    await prisma.broadcastCampaign
      .update({ where: { campaign_id: campaignId }, data: { status: "cancelled" } })
...
export async function cancelOrDeleteCampaign(campaignId, auditContext = {}) {
  try {
    const campaign = await prisma.broadcastCampaign.findUnique({
      where:   { campaign_id: campaignId },
      include: {
        attachmentFiles: { where: { attachment_type: { in: ["IMAGE", "VIDEO"] } } },
      },
    });

    if (!campaign)                  throw createError("Campaign not found", 404);
    if (campaign.status === "sent") throw createError("Cannot delete a sent campaign", 400);
    ...
    if (campaign.status === "scheduled") {
      ...
      }).catch((err) => console.error("[AUDIT] Failed to log inapp cancel:", err));

      return { campaign_id: updated.campaign_id, status: updated.status, message: "Scheduled campaign cancelled successfully" };
    }
    ...
    await prisma.broadcastCampaign.delete({ where: { campaign_id: campaignId } });
    ...
    }).catch((err) => console.error("[AUDIT] Failed to log inapp draft delete:", err));

    return { campaign_id: campaignId, message: "Campaign deleted successfully" };
  } catch (error) {
...
export async function getDrafts(cadminId, pagination = {}) {
  ...
  const [drafts, total] = await Promise.all([
    ...
    prisma.broadcastCampaign.count({ where: { created_by_cadmin: cadminId, status: "draft" } }),
  ]);
    ...
    campaigns.map(async (campaign) => {
      ...
      return {
        ...
        read_rate:       delivered > 0
          ? ((readCount / delivered) * 100).toFixed(1)
          : "0.0",
      };
    }),
  );
