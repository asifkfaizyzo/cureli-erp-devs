  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.marketplaceOrderStatusHistory.create({
      data: {
        ...
        reason:
          target_status === 'REJECTED'  ? reason :
          target_status === 'CANCELLED' ? actor_type :
          actor_type === 'system'       ? 'auto_completed' :
          null,
      },
    });
  ...
  for (const item of order.items) {
    ...
    if (firstImage) {
      imageUrl = firstImage.startsWith('medicine_images/')
        ...
        : resolveAssetUrl(`medicine_images/${listing.linkedVariant.sku_id}/${firstImage}`);
    }
    ...
    available.push({
      ...
      branchLatitude:       order.branch?.latitude  ? Number(order.branch.latitude)  : null,
      branchLongitude:      order.branch?.longitude ? Number(order.branch.longitude) : null,
    });
  }
...
function formatOrderItem(item) {
  return {
    ...
    image_url:              resolveOrderItemImageUrl(item.variant, item.variant_sku_snapshot),
  };
}
