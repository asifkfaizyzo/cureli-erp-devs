...
export async function createCheckoutSession({
  ...
  if (!branchSettings || !branchSettings.branch.is_active) throw new Error('Branch is not available');
  if (!branchSettings.marketplace_enabled) throw new Error('This branch is not accepting marketplace orders');
  ...
  if (requiresPrescription && prescription_files.length === 0) {
    throw new Error('This order requires a prescription. Please upload at least one prescription file.');
  }
...
export async function confirmCheckoutPayment({
  ...
  if (!session || session.customer_id !== customer_id) throw new Error('Session not found');
...
async function _createOrderFromSession({
  ...
  if (fresh?.order_id)    return prisma.marketplaceOrder.findUnique({ where: { order_id: fresh.order_id } });
  if (fresh?.status === 'paid') return prisma.marketplaceOrder.findUnique({ where: { order_id: fresh.order_id } });
...
async function validateAndSnapshotItems(cartItems, branch_id) {
  ...
  for (const { variantId, quantity } of cartItems) {
    ...
    if (!listing.is_visible)                  throw new Error('One or more items are no longer listed');
    if (listing.stock_status !== 'IN_STOCK')  throw new Error('One or more items are out of stock');
    if (listing.marketplace_price === null)   throw new Error('One or more items have no price set');

    if (listing.requires_prescription) requiresPrescription = true;

    const unitPrice  = Number(listing.marketplace_price);
    const mrp        = listing.linkedVariant?.mrp ? Number(listing.linkedVariant.mrp) : unitPrice;
