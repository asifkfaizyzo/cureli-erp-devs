...
import { formatCurrency, formatDate } from "./invoiceModalHelpers";
...
// ═══════════════════════════════════════════════════════════════════════════
// UTILITY: Safe Amount Parser & Free Item Detection
// ═══════════════════════════════════════════════════════════════════════════
...
function isFreeLine(item) {
  ...
  for (const val of totalCandidates) {
    const parsed = safeParseAmount(val);
    if (parsed !== null) {
  ...
  const taxable = safeParseAmount(item.taxable_amount);
  const freeQty = parseFloat(item.free_quantity) || 0;
  if (taxable !== null && taxable === 0 && freeQty > 0) return true;

  const qty = parseFloat(item.quantity) || 0;
  if (qty === 0 && freeQty > 0) return true;
...
function computeItemAmounts(item) {
  ...
  const tradeDsc = parseFloat(item.trade_discount) || 0;
  const schemeDsc = parseFloat(item.scheme_discount) || 0;
...
// ═══════════════════════════════════════════════════════════════════════════
// GENERATE PRINT HTML (with options support)
// ═══════════════════════════════════════════════════════════════════════════

function generatePrintHTML(invoice, company, totals, options) {
  ...
  headers.push('<th style="text-align:left;min-width:140px">Product</th>');
  if (options.showHSN) headers.push('<th style="width:55px">HSN</th>');
  if (options.showBatch) headers.push('<th style="width:65px">Batch</th>');
  if (options.showExpiry) headers.push('<th style="width:45px">Exp</th>');
  if (options.showPack) headers.push('<th style="width:35px">Pack</th>');
  headers.push('<th style="width:30px">Qty</th>');
  if (options.showFreeQty) headers.push('<th style="width:30px">Free</th>');
  if (options.showMRP) headers.push('<th style="width:55px">MRP</th>');
  headers.push('<th style="width:55px">Rate</th>');
  if (options.showDiscount) headers.push('<th style="width:40px">Disc</th>');
  if (options.showTaxable) headers.push('<th style="width:65px">Taxable</th>');
  if (options.showCGST) headers.push('<th style="width:58px">CGST</th>');
  if (options.showSGST) headers.push('<th style="width:58px">SGST</th>');
...
.badge{display:inline-block;padding:2px 10px;border-radius:20px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1px;margin-top:5px}
...
.item-summary .free-badge{color:#16a34a;font-weight:600}
...
<div class="wrap">
  ...
  <div class="hdr">
    ...
    <div class="inv-rt">
      ...
      <div class="badge ${statusClass}">${invoice?.status || "DRAFT"}</div>
    </div>
  </div>
  ...
  <div class="dgrid">
    ...
    <div class="dbox">
      ...
      ${supplier?.phone ? `<div class="dr"><span class="dl">Phone:</span><span class="dv">${supplier.phone}</span></div>` : ""}
      ${supplier?.email ? `<div class="dr"><span class="dl">Email:</span><span class="dv">${supplier.email}</span></div>` : ""}
  ...
  <div class="summ">
    ...
    <table class="tots">
      ...
      ${options.showBalanceAmount && paidAmount > 0 ? `<tr class="bal"><td class="tl">Balance:</td><td class="tv">₹${balance.toFixed(2)}</td></tr>` : ""}
    </table>
  </div>
...
const PreviewTab = ({ invoice, company, totals, options, shopLoading }) => {
  ...
  if (options.showFreeQty) visibleHeaders.push("Free");
    ...
    <div className="text-[11px] leading-snug text-gray-900 select-none">
      ...
      <div className="flex justify-between items-start border-b-[3px] border-[#000060] pb-3 mb-3">
        ...
        <div className="text-right">
          ...
          <span
            className={`inline-block mt-1.5 px-3 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider border ${
...
const PrintInvoiceModal = ({ open, onClose, invoice, companyInfo }) => {
  ...
  const totals = useMemo(() => {
    if (!invoice?.lineItems) return { subtotal: 0, cgst: 0, sgst: 0, total: 0 };
  ...
  const handlePrint = useCallback(() => {
    if (!invoice) return;
    const html = generatePrintHTML(invoice, company, totals, printOptions);
  ...
  const handleDownloadHTML = useCallback(() => {
    ...
    a.href = url;
    a.download = `Invoice-${invoice.invoice_number || "draft"}.html`;
