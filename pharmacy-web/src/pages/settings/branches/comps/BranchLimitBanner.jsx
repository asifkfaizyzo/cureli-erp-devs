...
import { Building2, AlertTriangle, Infinity } from "lucide-react";

/**
 * BranchLimitBanner
 * Shows current branch count vs plan limit
 */
const BranchLimitBanner = ({ limits }) => {
      ...
      >
        ...
        <div>
          ...
          <p className="text-xs text-emerald-600">
            {current_count} branch{current_count !== 1 ? "es" : ""} created
          </p>
        </div>
      </motion.div>
    );
  ...
  const Icon = isAtLimit ? AlertTriangle : isNearLimit ? AlertTriangle : Building2;
      ...
      <div className="flex-1">
        ...
        <p className={`text-xs mt-1 ${subTextColor}`}>
          {isAtLimit
            ...
            : `${remaining} branch${remaining !== 1 ? "es" : ""} remaining`
          }
        </p>
      </div>
    </motion.div>
...
export default BranchLimitBanner;