// frontend/src/pages/Settings/comps/ContactModal.jsx
...
const ContactModal = ({ currentEmail, currentPhone, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {};
    const trimEmail = email.trim().toLowerCase();
    if (trimEmail !== (currentEmail || "").toLowerCase()) payload.email        = trimEmail;
    ...
    try {
      ...
      setErrors({ general: err.response?.data?.message || "Failed to update contact info" });
    } finally {
      setSaving(false);
    }
  };
      ...
      >
        ...
        <div className="bg-gradient-to-r from-[#05015A] to-[#0a0280] px-6 py-4
          ...
          <div className="flex items-center gap-2 text-white">
            <Mail size={18} />
            <h3 className="text-base font-semibold">Edit Contact Information</h3>
          </div>
          <button
            onClick={() => !saving && onClose(false)}
            disabled={saving}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <div className={`flex items-center border rounded-xl overflow-hidden
              ...
              <span className="px-3 h-10 flex items-center bg-gray-50 text-sm
