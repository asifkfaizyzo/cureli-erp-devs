// frontend/src/pages/Settings/comps/IdentityModal.jsx

import { useState } from "react";
import { X, User, AtSign, Loader2, AlertCircle, Save, Crown } from "lucide-react";
...
const IdentityModal = ({ currentName, currentUsername, onClose }) => {
  ...
  const validate = () => {
    ...
    if (!nameChanged && !userChanged) { errs.general = "No changes detected"; return errs; }
    ...
    if (userChanged) {
      ...
      else if (lowerUser.length < 3)  errs.username = "Must be at least 3 characters";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const parts = [payload.name && "name", payload.username && "username"].filter(Boolean);
      onClose(`${parts.map((p) => p[0].toUpperCase() + p.slice(1)).join(" and ")} updated successfully`);
    } catch (err) {
      setErrors({ general: err.response?.data?.message || "Failed to update identity" });
    } finally {
      setSaving(false);
    }
  };
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                value={username}
                onChange={(e) => { setUsername(e.target.value.toLowerCase()); setErrors((p) => ({ ...p, username: undefined, general: undefined })); }}
