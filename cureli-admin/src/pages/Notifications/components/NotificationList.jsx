  ...
  return (
    ...
    >
      ...
      {isBroadcast ? (
        ...
        `}>
          <Megaphone size={14} className={isSelected || !is_read ? 'text-indigo-600' : 'text-gray-400'} />
        </div>
      ) : (
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5 min-w-0 flex-1">
            {/* Unread dot */}
            {!is_read && (
              <span className={`w-2 h-2 rounded-full flex-shrink-0 ${priorityConfig.dotColor}`} />
            )}
            
            <p className={`text-sm truncate ${is_read ? 'text-gray-700' : 'text-gray-900 font-medium'}`}>
              {title}
            </p>
          </div>
