...
const fakeBranchesForShop = (shopId) => {
  ...
  return Array.from({ length: 7 }).map((_, i) => ({
    ...
    city: ["Kochi", "Pune", "Mumbai", "Delhi", "Bangalore", "Calicut", "Kollam"][i % 7],
...
const BranchRow = ({ b, idx, onToggle }) => {
  return (
    <tr className={`border-b border-gray-100 transition-all duration-150 ${idx % 2 === 0 ? "bg-white" : "bg-gray-50"} hover:bg-indigo-50 ${!b.is_active ? "opacity-60" : ""}`}>
      ...
      <td className="p-3 text-center">
        <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${b.is_active ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}>
      ...
      <td className="p-2 text-center">
        ...
        <button onClick={() => onToggle(b)} className="p-1.5 rounded-lg text-gray-500 hover:text-orange-600 hover:bg-orange-50 transition-all">
...
const ShopBranchesTable = ({ shopId, rowsPerPage = 5 }) => {
  ...
  const pageBranches = branches.slice((page - 1) * rowsPerPage, page * rowsPerPage);
  ...
  const confirmToggle = () => {
    ...
    setTimeout(() => {
      setBranches((prev) => prev.map(b => b.branch_id === targetBranch.branch_id ? { ...b, is_active: !b.is_active } : b));
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-4">
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-sm font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-2"><GitBranch size={16} /> Branches ({total})</h4>
      </div>
      ...
      {branches.length === 0 ? (
        ...
        <>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  ...
                  <th className="text-left p-3 font-semibold text-gray-600">Branch Name</th>
              ...
              <tbody>
                {pageBranches.map((b, idx) => (
                  <BranchRow key={b.branch_id} b={b} idx={(page - 1) * rowsPerPage + idx} onToggle={handleToggle} />
                ))}
              </tbody>
            </table>
          </div>
          ...
          <div className="mt-3 flex items-center justify-between">
            <div className="text-sm text-gray-500">Showing {(page - 1) * rowsPerPage + 1} to {Math.min(page * rowsPerPage, total)} of {total} results</div>
      ...
      <ConfirmDialog
        isOpen={showConfirm}
        onClose={() => { setShowConfirm(false); setTargetBranch(null); }}
        onConfirm={confirmToggle}
        title={targetBranch?.is_active ? "Suspend Branch?" : "Activate Branch?"}
        message={targetBranch?.is_active ? `Suspend "${targetBranch?.branch_name}"?` : `Activate "${targetBranch?.branch_name}"?`}
